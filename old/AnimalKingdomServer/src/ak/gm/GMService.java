...
public class GMService extends GMOrderContext implements Service{

	private static Logger log = Logger.getLogger(GMService.class);
	// 测试数据
	private static final int SUCCESS = 1;
	private static final int FAILED = 0;
	private static final String GM_SERVER_IP = Platform.getConfiguration().getString(ConfigKeys.GM_SERVER_IP);
	private static final int GM_SERVER_PORT = Platform.getConfiguration().getInt(ConfigKeys.GM_SERVER_PORT);;
	@Override
	...
	public String sendMail(String arg0) {
		...
		int id = IdUtil.deCode(name.trim());
		PlayerEx player = Platform.getEntityManager().fetch("from PlayerEx where accountId = ? and exist = 0", id);
...
//邮件系统有改动，这个类型暂时注掉
	...
	private JSONArray getPlayerInfo(PlayerEx player) {
		...
		if (account != null) {
			...
			compensateDollar.put(JSONContant.ATT, "系统补偿至灯笼：" + account.getCompensateDollar());
	...
	public void startup() {
		try {
			this.run(this.getClass().getName(), Platform.getGameCode(), Platform.getGameId(), GM_SERVER_IP, GM_SERVER_PORT);
	...
	@Override
	public String getCompensateDollar(String arg0) {
		JSONObject obj = JSONObject.fromObject(arg0);
		JSONObject param = obj.getJSONObject(JSONContant.PARAM);
		String playerName = param.getString("player");
		int id = IdUtil.deCode(playerName.trim());
		PlayerServiceEx service = (PlayerServiceEx) Platform.getAppContext().get(PlayerService.class);
		PlayerEx player = service.loadPlayer(service.getPlayerIdByAccountId(id));
		JSONObject msg = new JSONObject();
		msg.put(JSONContant.OP, GMOpCodeSend.GET_COMPENSATE_DOLLAR_RESULT);
		msg.put(JSONContant.FROM, GMOrderContext.getServerFlag());
		msg.put(JSONContant.TO, obj.get(JSONContant.FROM));
		if(player != null){
			if(player.getAccount() == null) {
				Account accout = Platform.getEntityManager().find(Account.class, player.getAccountId());
				player.setAccount(accout);
			...
			JSONObject p = new JSONObject();
			p.put("result", SUCCESS);
			p.put("msg", account.getCompensateDollar());
			p.put("player", playerName);
			msg.put(JSONContant.PARAM, p);
			return msg.toString();
		}else{
			JSONObject p = new JSONObject();
			p.put("result", FAILED);
			p.put("msg", "no such player");
			msg.put(JSONContant.PARAM, p);
			return msg.toString();
		}
	}
	...
	@Override
	public String sendDollar(String arg0) {
		JSONObject obj = JSONObject.fromObject(arg0);
		JSONObject param = obj.getJSONObject(JSONContant.PARAM);
		String name = param.getString("name");
		...
		if(player != null){
			...
			if(player.getAccount() != null && player.getAccount().getCompensateDollar() < count){ 
				...
				if(!ret){
					...
					msg.put(JSONContant.PARAM, result);
				}
			}else {
				JSONObject error = new JSONObject();
				error.put("result", FAILED);
	...
	@Override
	public String getOrders(String arg0) {
		String client = JSONObject.fromObject(arg0).getJSONObject(JSONContant.PARAM).getString("client");
		JSONObject obj = new JSONObject();
		List<Integer> orders = GMOrderContext.getOrders();
		JSONArray array = new JSONArray();
		JSONObject param = new JSONObject();
		for(int i=0;i<orders.size();i++){
			try{
				JSONObject order = new JSONObject();
				order.accumulate(JSONContant.ORDER, orders.get(i));
				array.add(order);
			}catch (Exception e) {
		...
		try{
			obj.put(JSONContant.OP, GMOpCodeSend.GET_ORDERS_RESULT);
			//将gamecode和serverid发给GMServer，在GMserver那注册session
			obj.put(JSONContant.FROM, GMOrderContext.getServerFlag());
			obj.put(JSONContant.TO, Contant.GM);
			param.put(JSONContant.ORDERLIST, array);
			param.put("client", client);
			obj.put(JSONContant.PARAM, param);
		}catch (Exception e) {
			log.error(e.toString(), e);
		}
		return obj.toString();
