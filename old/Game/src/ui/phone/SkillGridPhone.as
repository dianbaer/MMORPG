package ui.phone
{
	...
	public class SkillGridPhone extends Grid implements IAnimatable
	{
		...
		public function SkillGridPhone()
		{
			super();
			cdMask = new Bitmap();
			...
			if(bool){
				...
				if(isInCD){
					...
					if(time >= endTime){
						...
						cdMask.bitmapData = SkillUIPhone.arrCache[0];
						cdMask.visible = true;
					}else{
						return;
					}
		...
		public function onFrame():void{
			if(isInCD){
				...
				if(time >= cdTime){
					...
					var pos:int = Math.round(SkillUIPhone.arrCache.length*time/cdTime);
					if(pos >= SkillUIPhone.arrCache.length){
						pos = SkillUIPhone.arrCache.length-1;
					}
					cdMask.visible = true;
					cdMask.bitmapData = SkillUIPhone.arrCache[pos];
				}
			}else{
				if(target && target.isDispose){
					target = null;
