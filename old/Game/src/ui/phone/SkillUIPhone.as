package ui.phone
{
	...
	public class SkillUIPhone extends Box
	{
		...
		private static var W:int = 66;
		private static var H:int = 66;
		public static var arrCache:Vector.<BitmapData>;
		private var target:ActivityThing;
		public function SkillUIPhone()
		{
			super();
			drawMask();
			for(var i:int = 0;i<keyArrayShow.length;i++){
				var grid : SkillGridPhone = new SkillGridPhone();
				grid.setSize( 70, 70 );
				grid.x = i*80;
				grid.y = 0;
				addChild( grid );
		...
		private function onClick(event:MouseEvent):void{
			var grid : SkillGridPhone = event.currentTarget as SkillGridPhone;
			if(grid.data == null){
				return;
			}
			//取消技能
		...
		public function setSkill(skillArray:Array):void{
			...
			for(var i:int = 0;i<gridArray.length;i++){
				var grid : SkillGridPhone = gridArray[i];
		...
		public function useSkill(num:int):int{
			...
			if(index == -1){
				...
				var grid : SkillGridPhone = gridArray[index];
				if(grid.data == null){
					return -1;
				}
				//取消技能
